...
//    查询所有权限
    public List<Map> findResource() {
        List<Resource> resources = resourceMapper.selectAll();


        return getList(resources, 0);
    }
    ...
    private List<Map> getList(List<Resource> resources, Integer parentId){
        List<Map> mapList = new ArrayList<Map>();
        for (Resource resource : resources) {
            Map<String, Object> map = new HashMap<String, Object>();
            if(parentId.equals(resource.getParentId())) {
                map.put("path", resource.getId());
                map.put("resKey", resource.getResKey());
                map.put("resName", resource.getResName());
                map.put("children", getList(resources, resource.getId()));
                mapList.add(map);
            }
